...
func simpleTextQuery(b *gocb.Bucket) {
	indexName := "name_list"
	query := gocb.NewSearchQuery(indexName, cbft.NewMatchQuery("John")).
		Limit(10)
...
func docIdQueryMethod(b *gocb.Bucket) {
	indexName := "name_list"
	query := gocb.NewSearchQuery(indexName, cbft.NewDocIdQuery("u:23"))
...
func queryStringMethod(b *gocb.Bucket) {
	indexName := "name_list"
	query := gocb.NewSearchQuery(indexName, cbft.NewQueryStringQuery("employee :Bernard")).
		Limit(10)
...
func wildCardQueryMethod(b *gocb.Bucket) {
	indexName := "name_list"
	query := gocb.NewSearchQuery(indexName, cbft.NewWildcardQuery("Coll*").Field("myEmployee")).
		Limit(10).Highlight(gocb.DefaultHighlightStyle)
